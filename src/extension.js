export default class Carousel {
    ...
    init() {
        ...
        if (this.items.length <= 1) {
            letcarouselContainer = this.element.querySelector('.carousel-container');
     ...
     */
    _initOrder() {
        letcurrentActiveItem = this.element.querySelector('.carousel-item.is-active');
        if (!currentActiveItem) {
            this.items[0].classList.add('is-active');
            currentActiveItem = this.items[0];
        }
        letcurrentActiveItemPos = this.items.indexOf(currentActiveItem);
        letlength = this.items.length;
